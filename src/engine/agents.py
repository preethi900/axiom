...
class SoftwareTester:
    ...
    def __init__(self, target_host: str, api_key: str = None):
        ...
        self.prompt = ChatPromptTemplate.from_messages([
            ("system", f"You are a Senior SDET. Write a high-quality, robust Pytest function for the following test scenario. The base URL is defined as `BASE_URL = '{target_host}'`. Use `httpx` for requests. Write SYNCHRONOUS test functions exclusively (no `async def`). Include assertion messages. \n\nIMPORTANT: When using f-strings for assertions, ensure you handle quotes correctly. If you access a dictionary with quotes inside an f-string, you MUST use a different quote type for the outer string. Output valid JSON."),
...
class SpecGenerator:
    """
    Agent responsible for generating a detailed Markdown specification from a simple user story.
    """
    def __init__(self, api_key: str = None):
        self.llm = get_llm(api_key)
        from langchain_core.output_parsers import StrOutputParser
        ...
        self.prompt = ChatPromptTemplate.from_messages([
            ("system", "You are an expert Technical Product Manager. "
                       "Convert the user's brief story into a comprehensive Markdown specification ready for development and testing. "
                       "You MUST strictly follow these formatting guidelines if provided:\n\n{guidelines}\n\n"
                       "Output ONLY valid raw markdown without additional conversational filler."),
            ("user", "User Story: {story}")
        ])

    def generate(self, story: str, guidelines: str = "") -> str:
        chain = self.prompt | self.llm | self.parser
        ...
        try:
            ...
            import traceback
            traceback.print_exc()
            print(f"LLM Call failed: {e}. Returning fallback spec.", file=sys.stderr)
            return f"# Generated Specification\n\n## User Story\n{story}\n\n## Acceptance Criteria\n### AC-01: Auto-generated\n**Given** a basic setup\n**When** executed\n**Then** it works\n"
